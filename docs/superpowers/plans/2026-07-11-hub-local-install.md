...
- [x] `GET /health` 在应用运行时返回 ok
- [x] 合法 zip → 导入成功、窗口聚焦、详情打开、toast
- [x] 非法 URL / 坏包 / 下载失败返回规格中的状态码与 `error` 字段
- [x] 并发 install 返回 409
- [x] 临时目录不残留
- [x] Hub 端规格仍由 Hub 仓库单独实现（本计划不包含）
