...
将 Autoforge 的 Hub 登录改为浏览器授权码流程：用户在 Autoforge 中发起登录，在 Hub 网页登录并确认授权，随后自动回到桌面端。桌面端展示已登录账户信息，在设置中提供账户模块，并将脚本中心重构为与 Autoforge 主题一致的工具工作台。
...
授权作用域固定为读取用户资料、读取有权访问的脚本/团队和申请一次性安装令牌；不授予账户编辑、上传、发布、团队管理或删除权限。
...
- 已登录态顶部显示头像、昵称、邮箱和团队数量；左侧导航为市场、个人和团队；中部为搜索、分类、语言、排序、分页脚本列表；右侧为详情、README 和安装操作。
...
- UI：测试未登录不展示密码、授权等待可取消、已登录账户摘要、账户管理外链、退出、市场/个人/团队脚本状态。
