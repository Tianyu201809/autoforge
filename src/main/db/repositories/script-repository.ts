...
const SCRIPT_JOIN = `
  SELECT
    ...
    s.entry, s.language, s.imported_at, s.env_schema, s.param_schema, s.dependencies, s.browser,
    ...
    this.db
      .prepare(
        `INSERT INTO scripts (
          ...
          icon, icon_color, icon_bg, icon_border, version, entry, language, imported_at, env_schema, param_schema,
      ...
      this.db
        .prepare(
          `UPDATE scripts SET
            ...
            version = @version, entry = @entry, language = @language, imported_at = @imported_at, env_schema = @env_schema, param_schema = @param_schema,
