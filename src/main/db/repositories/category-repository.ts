...
export class CategoryRepository {
  ...
  listCategories(): StoredCategory[] {
    ...
    return rows.map((row) =>
      rowToStoredCategory(row as { id: string; key: string; label: string; color_preset: string })
    )
  }
  ...
  insertCategory(category: StoredCategory): void {
    this.db
      .prepare('INSERT INTO categories (id, key, label, color_preset) VALUES (@id, @key, @label, @color_preset)')
      .run({
        id: category.id,
        key: category.key,
  ...
  updateCategory(id: string, patch: { label?: string; colorPreset?: string }): StoredCategory | null {
    const row = this.db.prepare('SELECT * FROM categories WHERE id = ?').get(id) as
      | { id: string; key: string; label: string; color_preset: string }
      | undefined
    if (!row) return null
    ...
    this.db
      .prepare('UPDATE categories SET label = @label, color_preset = @color_preset WHERE id = @id')
      .run({ id: next.id, label: next.label, color_preset: next.colorPreset })
    return next
  }
